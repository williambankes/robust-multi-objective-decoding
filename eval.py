...
class EvalModule(pl.LightningModule):
    ...
    def test_step(self, batch, batch_idx):
        ...
        decoder_output = self.decoder.generate(
            ...
            attention_mask=batch['prompt_attention_mask'], #Added for the blockwise approach
        ...
        response_batch = [self.remove_prompt_from_response(p, r) for p, r in zip(prompt_batch, response_batch)]
        ...
        for i, response in enumerate(response_batch):
            ...
            if isinstance(decoder_output, dict):
                for k,v in decoder_output.items(): #TODO: make this a parsed argument to the eval function
                    if k in ['is_cbf_condition_met', 'is_safe_record', 'joint_branch_probs_record']: # handle the list outputs
                        output_dict[k] = v[:,i]
            ...
            if evals is not None:
                if isinstance(evals[0], list):
                    ...
                    raise NotImplementedError(f'Unexpected evals type {type(evals[0])}, for evals: {evals}')
                
            if self.eval_test_response:
                output_dict['value_eval_test_response'] = value_eval[i].cpu().to(torch.float32).numpy()
                if self.save_q_values:
                    output_dict['q_value_eval_test_response'] = q_value_eval[i].cpu().to(torch.float32).numpy()
...
def eval(config: DictConfig):
    ...
    ref_tokenizer = AutoTokenizer.from_pretrained(
        config.ref_model.pretrained_model_name_or_path,
        padding_side='left'
    )
    ...
    collate_func = hydra.utils.instantiate(
        config.collate_fn, tokenizer=ref_tokenizer, max_length=config.tokenizer.max_length
    )
    ...
    if config.checkpoint_path is not None:
        ...
        state_dict = load_base_vf_module_state_dict_from_checkpoint(config.checkpoint_path)
    ...
    if config.score_and_generate:                     
        eval_module = EvalModule(decoder, oracle, ref_tokenizer, max_new_tokens=config.max_new_tokens,
                                 eval_test_response=config.get('eval_test_response', False))
    else:
        eval_module = EvalModule(decoder, None, ref_tokenizer, max_new_tokens=config.max_new_tokens,
                                eval_test_response=config.get('eval_test_response', False))
    ...
    try:
        ...
        assert "DefaultCPUAllocator: can't allocate memory" in str(e), 'Unexpected RuntimeError'
        
    # Save the outputs -> using datetime to create a unique file name
    time_file_name = datetime.now().strftime("%m-%d-%Y-%H-%M-%S")
    experiment_name = config.experiment_name if config.experiment_name is not None else '' 
    path = os.path.join('eval_outputs', f"eval_outputs_{experiment_name}_{time_file_name}.pt")
