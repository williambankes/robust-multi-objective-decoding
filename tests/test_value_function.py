...
from robust_multi_objective_decoding.value_function import (
    ValueFunctionModule,
    ActionValueFunctionModule,
)
...
def test_valuefunctionmodule_trainable_params(
    model_and_tokenizer, base_value_function_module, lora_config
):
    model, _ = model_and_tokenizer
    ...
    value_function_trainable_params = set(
        ...
        for name, param in base_value_function_module.named_parameters()
    ...
    with torch.no_grad():
        for name, param in learner_params.items():
            ...
            if "q_hat_head" in name or "q_hat_lora_adapter" in name:
                source_name = (
                    "value_function_module."
                    + base_value_function_module.polyak_update_mapping[
                ...
                manual_polyak = (
                    ...
                    * (1 - base_value_function_module.polyak_coeff)
...
def test_valuefunctionmodule_get_target_values(value_function_module):
    ...
    qs = value_function_module.get_target_values(
...
def test_actionvaluefunctionmodule_get_target_values(action_value_function_module):
    ...
    q_hat = action_value_function_module.get_target_values(
        input_ids, attention_mask=attention_mask
    )
