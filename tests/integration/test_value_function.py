...
def test_valuefunctionlearner_on_after_backward(learner):
    ...
    original_params = {name: param.detach().clone() for name, param in learner.named_parameters()}
    ...
    with torch.no_grad():
        for name, param in learner_params.items():
            ...
            if "target" in name:
                source_name = "base_value_function_module." + learner.base_value_function_module.polyak_update_mapping[name.removeprefix("base_value_function_module.")]
                source_param = learner_params[source_name]
                manual_polyak = (
                    original_params[name] * (1 - learner.base_value_function_module.polyak_coeff) 
...
def test_valuefunctionlearner_training(learner, dataloader):
    ...
    for name, param in learner_params.items():
        if param.requires_grad:
            assert not torch.allclose(param, original_params[name]), f"Parameter {name} has not been updated (original value: {original_params[name]}, new value: {param})"