...
def test_dataset_oversampling():
    ...
    original_safe = [
        i
        for i in range(len(more_imbalanced_data))
    ...
    for idx in original_safe:
        ...
        for i in range(len(dataset)):
            if (
                dataset.data[i]["prompt"] == orig_sample["prompt"]
                and dataset.data[i]["response_0"] == orig_sample["response_0"]
            ):
                found = True
                break
        assert (
            found
        ), f"Original safe sample at index {idx} not found in oversampled dataset"
...
def test_combined_balancing():
    ...
    safe_count_dataset = sum(
        1
        for i in range(len(dataset))
        if dataset.data[i]["combined_harm_label"][-1] == 1
    )
    unsafe_count_dataset = sum(
        1
        for i in range(len(dataset))
        if dataset.data[i]["combined_harm_label"][-1] == 0
    )
    assert (
        safe_count_dataset == unsafe_count_dataset
