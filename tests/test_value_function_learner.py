...
def test_mlp_head_layer_norm():
    ...
    head = MLPHead(input_dim=10, hidden_dim=20, output_dim=1, use_layer_norm=True)
    ...
    head_no_norm = MLPHead(
        input_dim=10, hidden_dim=20, output_dim=1, use_layer_norm=False
    )
...
def test_mlp_head_xavier_init():
    ...
    head = MLPHead(input_dim=10, hidden_dim=20, output_dim=1, xavier_init=True)
    ...
    head_no_xavier = MLPHead(
        input_dim=10, hidden_dim=20, output_dim=1, xavier_init=False
    )

    # Non-xavier weights should generally have larger values
    assert torch.mean(torch.abs(head.fc1.weight)) < torch.mean(
        torch.abs(head_no_xavier.fc1.weight)
    )
...
def test_mlp_head_sigmoid():
    ...
    head_unscaled = MLPHead(
        input_dim=10, hidden_dim=20, output_dim=1, use_sigmoid=True, scale_sigmoid=False
    )
...
class ProxyValueFunctionModule:
    ...
    def __call__(self, *args, **kwargs):
        return (torch.tensor([[1.0, 2.0, 3.0, 4.0]]), torch.tensor([[1.0, 2.0, 3.0]]))
...
def test_valuefunctionlearner_process_batch_for_iql_loss(value_function_learner):
    ...
    transition_safety_labels, v, q, q_hat = (
        value_function_learner.process_batch_for_iql_loss(batch)
    )
...
def test_valuefunctionlearner_cbf_mc_loss_term(value_function_learner_cbf_mc_loss):
    ...
    masked_loss_eval = value_function_learner_cbf_mc_loss.iql_loss(
        r, v, q, q_hat, gamma, tau
    )
...
def test_valuefunction_learner_mc_loss(value_function_learner_mc_loss):
    ...
    masked_loss_eval = value_function_learner_mc_loss.iql_loss(
        r, v, q, q_hat, gamma, tau
    )
...
def test_valuefunction_learner_tw_mc_loss(value_function_learner_tw_mc_loss):
    ...
    masked_loss_eval = value_function_learner_tw_mc_loss.iql_loss(
        r, v, q, q_hat, gamma, tau
    )
...
def test_valuefunctionlearner_configure_optimizers(value_function_learner):
    value_function_learner.parameters = nn.Sequential(
...
def test_valuefunctionlearner_on_after_backward(value_function_learner):
    ...
    param1 = value_function_learner.base_value_function_module.state_dict()[
        "layer1.lora_A.target_lora_adapter.weight"
    ]
