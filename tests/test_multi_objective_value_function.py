...
class ProxyBase(nn.Module):
    ...
    def forward(self, input_ids, attention_mask, output_hidden_states: bool):
        return ProxyOutput(
            [torch.randn(input_ids.shape[0], input_ids.shape[1], 10).to(torch.float16)]
        )
...
def test_MultiHeadValueFunction():
    ...
    lora_config = LoraConfig(
        r=16, lora_alpha=16, target_modules=["weights"], lora_dropout=0.1
    )
    ...
    assert (
        outputs.shape == (batch_size, num_rewards, seq_length)
    ), f"Expected shape {(batch_size, num_rewards, seq_length)}, but got {outputs.shape}"
