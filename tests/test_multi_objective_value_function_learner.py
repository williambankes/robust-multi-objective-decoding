...
from robust_multi_objective_decoding.value_function_learner import (
    MultiObjectiveValueFunctionLearner,
)
...
class ProxyBaseValueFunctionModule(nn.Module):
    ...
    def forward(self, input_ids, attention_mask):
        ...
        return torch.ones(
            input_ids.shape[0], self.num_outputs, input_ids.shape[1]
        ), torch.ones(input_ids.shape[0], self.num_outputs, input_ids.shape[1] - 1)
    ...
    def get_target_values(self, input_ids, attention_mask):
        ...
        """
        output = torch.ones(
            input_ids.shape[0], self.num_outputs, input_ids.shape[1] - 1
        )
...
def test_multiobj_process_batch_mispec_mask(multi_obj_value_function_learner):
    r = torch.concat(
        [
            torch.tensor([[[-100.0, -100.0, 0.0, 1.0]] * 3] * 3),
            torch.tensor([[[-100.0, 0.0, 0.0, 1.0]] * 2] * 3),
        ],
        dim=1,
    )
    ...
    with pytest.raises(AssertionError):
        rewards, v, q, v_hat, q_hat = multi_obj_value_function_learner.process_batch(
            batch
        )
    ...
    r = torch.concat(
        [
            torch.tensor([[[-100.0, -100.0, -100.0, -100.0]] * 3] * 3),
            torch.tensor([[[-100.0, 0.0, 0.0, 1.0]] * 2] * 3),
        ],
        dim=1,
    )
...
def test_multiobj_process_loss(multi_obj_value_function_learner):
    ...
    test_value = multi_obj_value_function_learner.process_losses(
        rewards=r,
        vs=v,
        qs=q,
        v_hats=v,
