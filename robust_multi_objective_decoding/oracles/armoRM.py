...
class ArmoRMOracle(Oracle, nn.Module):
   ...
   'code-style','code-explanation','code-instruction-following','code-readability']
    ...
    def __init__(
        ...
        if not debug:
            self.model = AutoModelForSequenceClassification.\
                from_pretrained("RLHFlow/ArmoRM-Llama3-8B-v0.1",
                                trust_remote_code=True,
                                torch_dtype=torch.bfloat16,
                                cache_dir=cache_dir, 
                                revision='main').half().to(device)
            self.model.eval()
    ...
    def score(self, prompt_batch:list[str],
        ...
        inputs = self.prep_prompt_response_input(prompt_batch, 
                                                 response_batch, 
                                                 eval_only_response=eval_only_response)
    ...
    def prep_prompt_response_input(self, prompts:List[str], 
                                   responses:List[str],
                                   eval_only_response:bool) -> torch.Tensor:
        """
        ...
        inputs = self.tokenizer.apply_chat_template(
            messages, truncation=True, padding=True, return_dict=True,
        ...
        for i, tokens in enumerate(inputs['input_ids']):
            if not self.assert_token_for_gating(tokens):
                inputs['input_ids'][i, -5:] = torch.tensor([128009, 128006, 78191, 128007, 271]).cpu() 
    ...
    def assert_token_for_gating(self, lst):
        ...
        token_pattern = torch.tensor([128009, 128006, 78191, 128007, 271]).to(lst.device)
