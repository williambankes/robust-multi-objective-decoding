...
class KaleidoOracle(Oracle, nn.Module):
    ...
    def __init__(
        self,
        model_name: str = "tsor13/kaleido-xl",
        mode="score",
        vrd_idx=[0],
        *args,
        **kwargs,
        ...
        for vrd in self.l_vrd:
            with torch.no_grad():
                if self.mode == "relevance":
                    relevance_v = self.model.get_relevance(
                        response, ["Value"] * len(response), [vrd] * len(response)
                    )
                    relevance_v = (
                        relevance_v[:, [0]].detach().cpu().float()
                    )  # pick the probability of relevance
                    l_scores.append(relevance_v)
                elif self.mode == "valence":
                    valence_v = self.model.get_valence(
                        response, ["Value"] * len(response), [vrd] * len(response)
                    )
                    valence_v = (
                        valence_v[:, [0]].detach().cpu().float()
                    )  # pick the probability of "support"
