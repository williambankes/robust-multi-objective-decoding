...
class HHHarmlessnessOracle(Oracle, nn.Module):
    ...
    def score(
        ...
        ), f"Number of prompts:{n_prompts} and responses: {n_responses} must be equal"

        messages = [
            prompt + response for prompt, response in zip(prompt_batch, response_batch)
        ]
        ...
        assert outputs.shape == (
            n_prompts,
            1,
        ), f"Expected output shape: {(n_prompts, 1)}, got: {outputs.shape}"
