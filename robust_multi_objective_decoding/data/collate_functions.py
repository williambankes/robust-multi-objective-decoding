...
def adjust_label_to_token_length(
    data: str, label: List[int], tokenizer: AutoTokenizer
) -> List[int]:
    """
    ...
    if indices.size > 0:
        ...
        adjusted_label = [1] * (split_shape - 1) + [
            0
        ] * diff  # subtract 1 to account for <bos> token

    # If there are only 0s in the label:
    else:
        tok_full_sentence = tokenizer([data], return_tensors="pt")
        adjusted_label = [0] * (
            len(tok_full_sentence["input_ids"][0]) - 1
        )  # subtract 1 to account for <bos> token
    ...
    def collate_fn(batch: List[Tuple]) -> Tuple[torch.Tensor]:
        ...
        for _, label in enumerate(labels):
            ...
            if length_l < max_size:
                output_labels.append(torch.tensor([-1] * (max_size - length_l) + label))
            elif length_l > max_size:
                output_labels.append(torch.tensor(label[:max_size]))
            else:
...
def create_word_break_tokenizer_collate_fn(tokenizer, max_length: int):
    ...
    def collate_fn(batch: List[Tuple]) -> Tuple[torch.Tensor]:
        ...
        tokenized_prompt_lens = [
            tokenizer(p, return_tensors="pt")["input_ids"].shape[-1] for p in prompt
        ]
        tokenized_response_lens = [
            tokenizer(r, return_tensors="pt")["input_ids"].shape[-1] for r in response
        ]
        ...
        labels = [
            adjust_label_to_token_length(response[i], label, tokenizer)
...
def create_eos_reward_tokenizer_rand_len_collate_fn(tokenizer, max_length: int):
    ...
    def collate_fn(batch: List[Tuple]) -> dict:
        ...
        response = [
            tokenizer(r, return_tensors="pt", truncation=True, max_length=rand_lens[i])
            for i, r in enumerate(response)
        ]
        response = [
            tokenizer.decode(r["input_ids"][0], skip_special_tokens=True)
            for r in response
        ]
        ...
        for i, label in enumerate(labels):
            ...
            padded_label = pad_safety_labels(
                eos_label,
                tokenized_response_lens[i],
                tokenized_prompt_lens[i],
                tokenizer.padding_side,
...
def create_word_break_tokenizer_rand_len_collate_fn(tokenizer, max_length: int):
    ...
    def collate_fn(batch: List[Tuple]) -> Tuple[torch.Tensor]:
        ...
        labels = [
            adjust_label_to_token_length(response[i], el, tokenizer)
...
def create_collate_functions(
    tokenizer, reward_collates: list, max_length: int, rand_len: bool
):
    """
