...
class BlockwiseRobustDecoder2(nn.Module):
    ...
    def _prepare_input_ids_for_vf(
        ...
        output_token_ids = [branches[i].outputs[0].token_ids for i in range(len(branches))]
    ...
    def _eval_value_function(self, input_ids: torch.Tensor, attention_mask: torch.Tensor):
        ...
        if self.value_function is not None:
            ...
            with torch.no_grad():
                values = self.oracle.score(prompt_batch, response_batch).to(torch.bfloat16)
    ...
    def _compute_weights(
        ...
        with torch.inference_mode(False):
            with torch.set_grad_enabled(True):
                optimal_group_weights_logits = torch.zeros([num_group, 1], requires_grad=True).to(adv.device)
                for iter in range(group_weights_iter):
                    optimal_group_weights_logits.retain_grad()
                    
                    optimal_group_weights = torch.softmax(
                        optimal_group_weights_logits, 
                        dim=0
                    ).to(torch.bfloat16)
                    ...
                    grad = torch.clamp(optimal_group_weights_logits.grad, min=-10, max=10)
    
                    optimal_group_weights_logits = optimal_group_weights_logits - (self.weight_step_size / torch.sqrt(torch.tensor(iter + 1))) * grad.view(num_group, 1)
                
        optimal_group_weights = torch.softmax(optimal_group_weights_logits, dim=0).view(num_group, 1)

        # Get importance sampling weights
        with torch.no_grad():
            is_weights_logs = torch.clamp(lambda_coef * adv.detach() @ optimal_group_weights.to(torch.bfloat16), max=max_grad)
            ...
            for i in range(num_iterations):
                ...
                branch_attention_mask = torch.concat([branch_attention_mask,
                                torch.ones(branch_attention_mask.shape[0],
                                            len_diff).to(branch_attention_mask.device)],
                                            dim=1)          
                # breakpoint()
                branch_attention_mask[:, -len_diff:][branches[:, -len_diff:] == self.ref_tokenizer.pad_token_id] = 0
                ...
                values = self._divided_evaluation(
                    branches, 
                ...
                attention_mask = torch.concat([attention_mask,
                                torch.ones(attention_mask.shape[0], len_diff).\
                                to(attention_mask.device)], dim=1)
                attention_mask[:, -len_diff:][branches[output_idxs, -len_diff:] == self.ref_tokenizer.pad_token_id] = 0

                input_ids = branches[output_idxs]
                input_ids[~unfinished_seqs, -len_diff:] = self.ref_tokenizer.pad_token_id
                for finished_token_id in self.tokens_finished:
                    unfinished_seqs = unfinished_seqs & (
                        (((finished_token_id == input_ids[:, -len_diff:]) * 1.0).sum(axis=1) == 0)
                    )
                
                assert len(unfinished_seqs.shape) == 1
                new_blocks = self.ref_tokenizer.batch_decode(
                    input_ids[:, -len_diff:]
                )
                ...
                prev_values = values[output_idxs].repeat_interleave(self.num_branches, dim=0)
