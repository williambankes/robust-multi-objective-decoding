...
def get_batch_logps(
    ...
    labels = labels[
        :, 1:
    ].clone()  # removes first token i.e. start of sentence token -> last label should be applied to penultimate logit, this does this.
    logits = logits[
        :, :-1, :
    ]  # removes last sequence element as such last label is applied to penultimate logit.
...
def separate_prompts_and_responses(
...
def kl_divergence(
    ...
    **kwargs,
    ...
    if not isinstance(model, type(reference_model)):
    ...
    with torch.no_grad():
        ...
        atten_mask = update_attention_mask(
            inputs["attention_mask"], samples, num_generations
        )
        ...
        per_token_logps = torch.gather(
            model_scores.log_softmax(-1), dim=2, index=labels.unsqueeze(2)
        ).squeeze(2)
        ...
        per_token_ref_logits = reference_model(
            input_ids=samples, attention_mask=atten_mask
        ).logits
    ...
    **kwargs,
    ...
    with torch.no_grad():
        ...
        model_gen = model.generate(
            **tokenized_prompts, max_new_tokens=max_new_tokens, do_sample=True
        )
        ref_model_gen = ref_model.generate(
            **tokenized_prompts, max_new_tokens=max_new_tokens, do_sample=True
        )
