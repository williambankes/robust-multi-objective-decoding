...
class MultiObjectiveValueFunctionLearner(pl.LightningModule):
    ...
    def process_batch(self, batch: Dict):
        ...
        assert (
            num_losses == self.base_value_function_module.get_number_outputs()
        ...
        assert (
            (mask_sum_across_rewards_dim == 0.0)
            + (mask_sum_across_rewards_dim == num_losses)
        ).all(), f"mask: {mask} must be consistent across the num_reward dimension"
        ...
        assert not (
            (mask[:, 0, :]).sum(dim=1) == 0
        ).all(), f"Entire batch of fully masked rows: {batch['idx']}, mask shape: {mask.shape},\
    ...
    def process_losses(
        self, input_ids, rewards, vs, qs, q_hats, v_hats, vs_next, split
    ) -> torch.Tensor:
        """
    ...
    def on_after_backward(self):
        ...
        """
        if (
            self.global_step % self.update_target_model_every == 0
            and self.is_polyak_average
        ):
